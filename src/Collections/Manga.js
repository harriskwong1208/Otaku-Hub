...
import { getCurrentUserId,checkUserWatchList ,checkUserMangaList} from "./Users";
...
async function addManga(manga){
    ...
    try{
        ...
        if(result === null){
            ...
            const mangaInfo = await axios.post(apiEndPoints.localHost+'manga',{
                "title": title,
                "mal_id": mal_id,
...
async function getMangaByMalId(mal_id){
    ...
    try{
        let manga = await axios.get(apiEndPoints.localHost+'manga');
...
async function getManga(id){
    let manga;
    try{
        manga = await axios.get(apiEndPoints.localHost+`manga/${id}`);
